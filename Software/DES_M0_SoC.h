...
// Struct for the SPI block
typedef struct 
{
	union
	{
		volatile uint8 spi_ctrl; // SPI control register
		volatile uint32  reserved0;	// overlapping 4-byte value
	};
	union 
	{
		volatile uint8 spi_tx_data; // transmit-data register in SPI master
		volatile uint32  reserved1;	// overlapping 4-byte value
	};
	union 
	{
		volatile uint8 spi_rx_data; // receive-data register in SPI master
...
// Macros for the SPI master's registers
