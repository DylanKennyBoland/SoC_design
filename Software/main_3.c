...
#define GPIO_SW_MASK 0x0003 // mask for the 16 GPIO switches
...
int main(void) 
{
    ...
    uint16 direction; // variable to store the state of the 16 GPIO switches
    int16 acceleration; // variable to store the acceleration
	...
	*/
	sendData2ADXL(POWER_CTL_ADDR, 0x0a); // configure the power control register
	sendData2ADXL(FILTER_CTL_ADDR, 0x03); // configure the filter control register
	delay(nLOOPS_per_DELAY); // small delay

	while(1) // The main loop
	{	
        // find out which axis the user wants to measure the acceleration along:
        direction = GPIO_SW;
        direction &= & GPIO_SW_MASK; // mask all switches bar the lowest two (most right two)
        switch (direction) { // read the correct registers based on the state of the two rightmost GPIO switches
            case 1u: // "u" for unsigned. 1 might correspond to x-axis
                acceleration = (int16)getHalfWordADXL(XDATA_L_ADDR);
                break;
            case 2u: // 2 might select the y-axis
                acceleration = (int16)getHalfWordADXL(YDATA_L_ADDR);
                break;
            case 3u: // 3 for the z-axis
                acceleration = (int16)getHalfWordADXL(ZDATA_L_ADDR);
                break;
            default:
                acceleration = (int16)getHalfWordADXL(XDATA_L_ADDR);
                break;
        }
        displayHalfWordNum(acceleration); // display the acceleration value
        delay(nLOOPS_per_DELAY); // small delay
